...
def includeme(config):
    ...
    config.add_route(
        "atramhasis.rdf_void_turtle_ext", pattern="/void.ttl", accept="text/turtle"
    )
    # The routes below extend the existing skosprovider routes with extensions
    config.add_route(
        "skosprovider.conceptscheme.cs.rdf", pattern="/conceptschemes/{scheme_id}/c.rdf"
    )
    config.add_route(
        "skosprovider.conceptscheme.cs.ttl", pattern="/conceptschemes/{scheme_id}/c.ttl"
    )
    config.add_route(
        "skosprovider.conceptscheme.rdf", pattern="/conceptschemes/{scheme_id}.rdf"
    )
    config.add_route(
        "skosprovider.conceptscheme.ttl", pattern="/conceptschemes/{scheme_id}.ttl"
    )
    config.add_route(
        "skosprovider.conceptscheme.csv", pattern="/conceptschemes/{scheme_id}/c.csv"
    )
    config.add_route(
        "skosprovider.c.rdf", pattern="/conceptschemes/{scheme_id}/c/{c_id:.*}.rdf"
    )
    config.add_route(
        "skosprovider.c.ttl", pattern="/conceptschemes/{scheme_id}/c/{c_id:.*}.ttl"
    )

    # tree
    config.add_route(
        "scheme_tree_html",
        pattern="/conceptschemes/{scheme_id}/tree",
        accept="text/html",
    )
    config.add_route(
        "scheme_tree",
    ...
    # language
    config.add_route(
        "atramhasis.list_languages",
        pattern="/languages",
        accept="application/json",
        request_method=("GET", "HEAD"),
    )
    config.add_route(
        "atramhasis.get_language",
        pattern="/languages/{l_id}",
        accept="application/json",
        request_method=("GET", "HEAD"),
    )
    config.add_route(
        "atramhasis.edit_language",
        pattern="/languages/{l_id}",
        accept="application/json",
        request_method="PUT",
    )
    config.add_route(
        "atramhasis.delete_language",
        pattern="/languages/{l_id}",
        accept="application/json",
    ...
    config.add_route(
        "tree_invalidate", pattern="/admin/tree/invalidate", accept="application/json"
    )
