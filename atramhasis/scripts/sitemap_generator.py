...
def create_sitemaps(settings, limit_per_deel, directory, env):
    ...
    for p in skos_registry.get_providers():
        ...
        concept_urls.extend(
            [concepts_url.format(scheme_id, x["id"]) for x in p.get_all()]
        )
...
def create_index_sitemap(base_url, directory):
    ...
    list_sitemaps = [
        ...
        and "sitemap" in f
        and "sitemap_index.xml" not in f
    ]
...
def main():  # pragma: no cover
    ...
    parser.add_argument("settings_file", help="<The location of the settings file>")
    parser.add_argument(
        "--limit", type=int, help="range of objects in sitemap", default=50000
    )
    parser.add_argument(
        "--no-input", action="store_true", help="Don't stop script for user input"
    )
