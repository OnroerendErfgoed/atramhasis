...
def void_dumper(request, registry):
    ...
    providers = [
        x
        for x in registry.get_providers()
        if not any(
            [not_shown in x.get_metadata()["subject"] for not_shown in ["external"]]
        )
    ]
    graph = Graph()
...
def _add_provider(graph, provider, dataseturi, request):
    ...
    duri = metadataset.get(
        "uri", request.route_url("atramhasis.rdf_void_turtle_ext", _anchor=pid)
    )
    pd = URIRef(duri)
    ...
    graph.add(
        (
            pd,
            FOAF.homepage,
            URIRef(request.route_url("skosprovider.conceptscheme", scheme_id=pid)),
        )
    )
...
def _add_metadataset(graph, subject, metadataset):
    ...
    """
    mapping = {
        "creator": {"predicate": DCTERMS.creator, "objecttype": URIRef},
        "publisher": {"predicate": DCTERMS.publisher, "objecttype": URIRef},
        "contributor": {"predicate": DCTERMS.contributor, "objecttype": URIRef},
        "language": {"predicate": DCTERMS.language},
        "date": {"predicte": DCTERMS.date},
        "created": {"predicate": DCTERMS.created},
        "issued": {"predicate": DCTERMS.issued},
        "license": {"predicate": DCTERMS.license, "objecttype": URIRef},
    }
