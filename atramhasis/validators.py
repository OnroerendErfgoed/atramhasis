...
class Note(colander.MappingSchema):
    note = colander.SchemaNode(colander.String(), preparer=html_preparer)
    type = colander.SchemaNode(colander.String())
    language = colander.SchemaNode(colander.String())


class Source(colander.MappingSchema):
    citation = colander.SchemaNode(colander.String(), preparer=html_preparer)
...
class RelatedConcept(colander.MappingSchema):
    id = colander.SchemaNode(ForcedString())
...
class MatchList(colander.SequenceSchema):
    match = colander.SchemaNode(colander.String(), missing=None)
...
class Concept(colander.MappingSchema):
    ...
    infer_concept_relations = colander.SchemaNode(
        colander.Boolean(), missing=colander.drop
    )
...
def collection_type_rule(
    errors, node_location, skos_manager, conceptscheme_id, members
):
    """
...
def semantic_relations_rule(
    errors, node_location, skos_manager, conceptscheme_id, members, collection_id
):
    """
...
def hierarchy_build(
    ...
    property_list,
    property_hierarchy,
    property_concept_type,
    property_list_name,
):
    """
    Build a list of all concept_ids reachable by recursively following a relationship.

    Walks the hierarchy starting from ``property_list`` and appends every
    reachable concept_id to ``property_hierarchy``.  A *visited* set prevents
    infinite loops and ensures each node is processed at most once.

    :param skos_manager: Manager used to look up things by id.
    :param conceptscheme_id: The conceptscheme to search within.
    :param list property_list: Starting concept_ids to traverse from.
    :param list property_hierarchy: Accumulator list for all reachable ids.
    :param str property_concept_type: Only follow nodes of this type
        (``'concept'`` or ``'collection'``), or ``None`` for any type.
    :param str property_list_name: The relationship attribute to follow
        (e.g. ``'narrower_concepts'``, ``'broader_concepts'``, ``'members'``,
        ``'member_of'``).
    """
    visited = set()
    queue = list(property_list)

    while queue:
        property_concept_id = queue.pop(0)
        if property_concept_id in visited:
            continue
        visited.add(property_concept_id)
        try:
            property_concept = skos_manager.get_thing(
                property_concept_id, conceptscheme_id
            )
        except NoResultFound:
            continue

        # The type filter controls whether we *recurse* from this node,
        # not whether we include its children. Children are always added
        # to the output when discovered (see below).
        if (
            property_concept_type is not None
            and property_concept.type != property_concept_type
        ):
            continue

        child_ids = [
            n.concept_id for n in getattr(property_concept, property_list_name)
        ]
        for child_id in child_ids:
            property_hierarchy.append(child_id)
...
def hierarchy_rule(
    ...
    if property2 in cstruct:
        ...
        if hasattr(skos_manager, "get_hierarchy_ids"):
            ...
            property2_hierarchy |= set(fallback_list)
    for broader_concept_id in property1_list:
        if broader_concept_id in property2_hierarchy:
            errors.append(colander.Invalid(node_location, error_message))


def broader_hierarchy_rule(
    errors, node_location, skos_manager, conceptscheme_id, cstruct
):
    """
...
def subordinate_arrays_only_in_concept_rule(
    errors, node, concept_type, subordinate_arrays
):
    """
...
def subordinate_arrays_type_rule(
    errors, node_location, skos_manager, conceptscheme_id, subordinate_arrays
):
    """
...
def subordinate_arrays_hierarchy_rule(
    errors, node_location, skos_manager, conceptscheme_id, cstruct
):
    """
...
def superordinates_type_rule(
    errors, node_location, skos_manager, conceptscheme_id, superordinates
):
    """
