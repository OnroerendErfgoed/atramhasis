...
class AtramhasisRDF:
    ...
    @audit
    @view_config(
        route_name="skosprovider.conceptscheme.cs", accept="application/rdf+xml"
    )
    ...
    @view_config(
        route_name="skosprovider.conceptscheme.cs", accept="application/x-turtle"
    )
    ...
    def rdf_conceptscheme_export(self):
        ...
        response.content_disposition = 'attachment; filename="{}.rdf"'.format(
            str(self.scheme_id)
        )
        return response
    ...
    def rdf_conceptscheme_export_turtle(self):
        ...
        response = Response(content_type="text/turtle")
        response.text = graph.serialize(format="turtle")
        response.content_disposition = 'attachment; filename="{}.ttl"'.format(
            str(self.scheme_id)
        )
        return response
    ...
    def get_conceptscheme_jsonld(self):
        ...
        response.content_disposition = 'attachment; filename="{}.jsonld"'.format(
            str(self.scheme_id)
        )
        return response
    ...
    @audit
    @view_config(
        route_name="skosprovider.c", permission="view", accept="application/ld+json"
    )
