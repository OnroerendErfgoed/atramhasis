...
def sort_by_labels(concepts, locale, reverse=False):
    return sorted(
        (x for x in concepts if x.label(locale)),
        reverse=reverse,
...
def get_public_conceptschemes(skos_registry):
    ...
    """
    conceptschemes = [
        {"id": x.get_metadata()["id"], "conceptscheme": x.concept_scheme}
        for x in skos_registry.get_providers()
        if not any(
            [
                not_shown in x.get_metadata()["subject"]
...
class AtramhasisView:
    ...
    def favicon_view(self):
        ...
        response = FileResponse(icon, request=self.request, content_type="image/x-icon")
        return response
    ...
    @view_config(
        ...
        renderer="atramhasis:templates/conceptschemes.jinja2",
    )
    ...
    @view_config(
        ...
        renderer="atramhasis:templates/conceptscheme.jinja2",
    )
    def conceptscheme_view(self):
        ...
        title = (
            conceptscheme.label(locale).label if (conceptscheme.label()) else scheme_id
        )
    ...
    @audit
    @view_config(
        route_name="skosprovider.c", renderer="atramhasis:templates/concept.jinja2"
    )
    def concept_view(self):
        ...
        try:
            ...
            url = self.request.route_url(
                "skosprovider.c", scheme_id=scheme_id, c_id=c_id
            )
            update_last_visited_concepts(
                self.request, {"label": c.label(locale).label, "url": url}
            )
            return {
                "concept": c,
    ...
    @view_config(
        ...
        renderer="atramhasis:templates/search_result.jinja2",
    )
    ...
    def set_locale_cookie(self):
        ...
        response.set_cookie(
            "_LOCALE_", value=language, max_age=31536000
        )  # max_age = year
        return response
                ...
                if concept.type == "concept":
                    rows.append(
                        (
                            ...
                            labels_to_string(concept.labels, "prefLabel"),
                            labels_to_string(concept.labels, "altLabel"),
                            get_definition(concept.notes),
                            [c.concept_id for c in concept.broader_concepts],
                    ...
                    rows.append(
                        (
                            ...
                            get_definition(concept.notes),
                            "",
                            [c.concept_id for c in concept.members],
                            "",
                        )
                    )
        return {"header": header, "rows": rows, "filename": "atramhasis_export"}
    ...
    def get_scheme(self, scheme, locale):
        ...
        if provider:
            ...
            tco = self.conceptscheme_manager.get_concepts_for_scheme_tree(
                conceptscheme_id
            )
            tcl = self.conceptscheme_manager.get_collections_for_scheme_tree(
                conceptscheme_id
            )
    ...
    def parse_thing(self, thing, parent_tree_id):
        ...
        if thing.type and thing.type == "collection":
            cs = (
                [member for member in thing.members]
...
class AtramhasisListView:
    ...
    def get_list(self, listtype):
        return [
            {"key": ltype.name, "label": self.localizer.translate(self._(ltype.name))}
