...
def check_trees(session, conceptscheme_label):
    sql_prov = SQLAlchemyProvider({"id": "TREES", "conceptscheme_id": 1}, session)
...
def check_menu(session, uri_pattern=None):
    ...
    sql_prov = SQLAlchemyProvider({"id": "MENU", "conceptscheme_id": 1}, session)
...
def check_parrots(session):
    sql_prov = SQLAlchemyProvider({"id": "PARROTS", "conceptscheme_id": 1}, session)
